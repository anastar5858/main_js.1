...
//function laptop instruction

const laptopInstructionsProvider = () => {
    console.log("please enter initiateGame() to start!!")
    alert(`You have to use the console to check for results I'm so evil.\n\n` + `You can use any of the following options to open the console tada! sometimes i'm nice:\n` + `\t1. Click the three-dot icon in the top right corner of the browser window.` + `From there, choose More Tools > Developer Tools. navigate to the console tab.\n` + `\t2. Press (Option+Command+i) on mac or (Ctrl+Shift+I) on Windows to` + ` open the developer tools and navigate to console tab.\n\n` + `Finally enter the command 'initiateGame()' ahh lot of steps`);
    askedIndicator = true;
    return
}
...
// function validate user choice
const validateUserChoice = () => {
    let userChoice = prompt('Choose your option (Rock, Paper, Scissors) or user (R, P, S).') ?. toLowerCase().trim();
    userChoice === 'r' ? userChoice = 'rock' : userChoice === 's' ? userChoice = 'scissors' : userChoice === 'p' ? userChoice = 'paper' : userChoice = userChoice;
...
// function initiate a new game

const initiateGame = () => {
    scores.computer = 0;
    scores.player = 0;

    if ((userDevice === 'laptop' || userDevice === 'l') && askedIndicator === false) 
        return laptopInstructionsProvider();
    ...
    if (askedIndicator === true || (userDevice !== 'laptop' && userDevice !== 'l')) {
        ...
        if (userDevice === 'laptop' || userDevice === 'l') 
            alert(`You can always try to conquer me again using initiateGame()`);
...
const playGame = () => {
    displayMessage(`YOU ARE AT ===== ROUND-${round} =====`)
    userChoice = validateUserChoice();
    if (userChoice === undefined) {
        displayMessage(interfaceMessages.cannotForfeit);
        const quit = confirm(`Unless you really want to quit :( don't make the AI cry.` + `You'll have to reload or manually restart haha`);
        if (quit) 
            return;
        
    }
    let computerChoice = computerRandomChoice();
    computerChoice === 'r' ? computerChoice = 'rock' : computerChoice === 's' ? computerChoice = 'scissors' : computerChoice === 'p' ? computerChoice = 'paper' : computerChoice = computerChoice;
